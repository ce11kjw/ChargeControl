...
**从局域网其他设备访问：**  
前端 API 请求默认指向 `http://127.0.0.1:8080`，从其他设备访问时需通过 URL 参数或 localStorage 指定设备 IP：

```
# 方式一：URL 参数（首次访问后自动记住）
http://<设备IP>:8080/?api=http://<设备IP>:8080

# 方式二：在浏览器控制台中设置（一次性）
localStorage.setItem('cc-api-base', 'http://<设备IP>:8080')
```
